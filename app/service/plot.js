...
class PlotService extends Service {
  ...
  async index(payload) {
    ...
    if(isPaging) {
      if(search) {
        res = await this.ctx.model.Plot.find({mobile: { $regex: search } }).populate('role').skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 }).exec()
        count = res.length
      } else {
        res = await this.ctx.model.Plot.find({}).populate('role').skip(skip).limit(Number(pageSize)).sort({ createdAt: -1 }).exec()
        count = await this.ctx.model.Plot.count({}).exec()
      }
    } else {
      if(search) {
        res = await this.ctx.model.Plot.find({mobile: { $regex: search } }).populate('role').sort({ createdAt: -1 }).exec()
        count = res.length
      } else {
        res = await this.ctx.model.Plot.find({}).populate('role').sort({ createdAt: -1 }).exec()
        count = await this.ctx.model.Plot.count({}).exec()
      }
    }
